...
Adapted from DietrichGebert/DietrichGebert.
"""
import html
import json
import os
...
USER = "nejcc"
JOINED_YEAR = 2013  # account creation year, never changes
...
# GITHUB_TOKEN (Actions) yields public contribution counts; a PAT in ACCESS_TOKEN
# (optional) additionally sees private repos. Either falls back to the other.
...
def info_lines(s):
    ...
    return [
        ...
        [],
        kv("OS", "Arch Linux"),
        kv("Uptime", "23+ years in software"),
        kv("Host", "LaravelPlus"),
        kv("Kernel", "Software Architect / CTO"),
        kv("IDE", "Claude Code, PhpStorm, VS Code"),
        [],
        kv("Languages.Programming", "PHP, Go, C, Python"),
        kv("Languages.Real", "Slovenian, English, Croatian"),
        [],
        rule("Contact"),
        kv("Email", "nejc.cotic@gmail.com"),
        kv("Web", "nejcc.github.io"),
        kv("X", "@lordneic"),
        kv("Writing", "dev.to/lordneic"),
        [],
        rule("GitHub Stats"),
...
def render(mode, stats):
    ...
    for i, line in enumerate(ART.strip("\n").split("\n")):
        out.append(f'<text x="25" y="{110 + i * 15}" fill="{p["art"]}" xml:space="preserve">{html.escape(line)}</text>')
    for i, segs in enumerate(info_lines(stats)):
        if not segs:
            continue
        spans = "".join(f'<tspan fill="{p[c]}">{html.escape(t)}</tspan>' for t, c in segs)
        out.append(f'<text x="330" y="{45 + i * 21}" xml:space="preserve">{spans}</text>')
    out.append("</svg>")
    return "\n".join(out)
